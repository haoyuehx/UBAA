...
UBAA 是 Kotlin Multiplatform 全栈同仓项目。核心代码分为 `shared`、`composeApp`、`server`、`androidApp` 和 `iosApp`，其中 `shared` 是前后端契约和跨平台业务接入层的中心。
...
- `androidApp`：Android 壳工程，承载 Compose Activity 和 Android 打包配置。
